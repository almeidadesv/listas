...
function listarProdutosPorRotatividade (rotatividade) {
    let listarRotatividade = produtos.filter ((produto) => 
      produto.rotatividade == rotatividade.toLowerCase())
    return listarRotatividade;
}
...
function listarRotatividadeECurva (rotatividade, curva) {
  let RotatividadeECurva = produtos.filter ((produto) => 
    produto.rotatividade == rotatividade && produto.curva_abc == curva)
  return RotatividadeECurva
}

console.table (listarRotatividadeECurva ("alta", "B"))

// (5 já feito e 6 em espera) exercicio 7

function promocaoCurvaC () {
let RotatividadeECurva = produtos.filter ((produto) => 
    produto.rotatividade == "baixa" && produto.curva_abc == "C").map
((produto) => ({... produto, promocao: produto.preco_venda * 0.9}))
  return RotatividadeECurva
}

console.table (promocaoCurvaC())
