...
//push adiciona um novo item
// pop apaga o último item adicionado
//splice esvazia mas não exclui

//produtos.splice (13, 1)

let nomes = produtos.map(produto => produto.nome)

let descontos = produtos.map((produto => ({... produto, comDesconto: produto.preco_venda * 0.9})))

//rotatividade alta, curva tipo A, estoque menor ou igual a 50
...
let custoEstoque = produtos.reduce (
    (acumulador, produto) => acumulador + (produto.preco_compra * produto.estoque) , 0
)
console.log ("O custo de todo o estoque foi de: " + custoEstoque.toLocaleString("pt-BR", {style: "currency", currency: "BRL" }));

let lucro = produtos.reduce (
    (acumulador, produto) => acumulador + (produto.preco_venda * produto.estoque) , 0
);
console.log ("O lucro de toda venda foi de: " + lucro.toLocaleString("pt-BR", {style: "currency", currency: "BRL" }));

let totalEstoque = produtos.reduce (
    (acumulador, produto) => acumulador + (produto.estoque) , 0
)
console.log ("A quantidade total de produtos em estoque é: " + totalEstoque)


let mediaEstoque = produtos.reduce (
    (acumulador, produto) => acumulador )
